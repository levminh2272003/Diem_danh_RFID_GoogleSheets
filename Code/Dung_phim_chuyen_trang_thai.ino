...
StudentStatus* getStudentStatus(const char* studentID) {
  ...
  for (int i = 0; i < MAX_STUDENTS; i++) {
    if (studentStatus[i].student_id[0] == '\0') { 
...
void loop() {
  ...
  /*Da doi thay vi bam button thi bam phim D
  ...
  */
  ...
  static bool doiXacNhanChuyenTrangThai = false;
  static unsigned long thoiGianXacNhanTrangThai = 0;
  ...
  if (key) {
    ...
    if (doiXacNhanReset) {
    ...
    else if (doiXacNhanChuyenTrangThai) {
      if (key == '#') {
        InOutState = !InOutState;
        digitalWrite(LED_IO, InOutState);
        display.print("Trang thai da doi");
        showDefaultScreen();
      } else {
        display.print("Huy doi trang thai");
      }
      doiXacNhanChuyenTrangThai = false;
    }
      ...
      display.print("Nhan # de doi TT");
      doiXacNhanChuyenTrangThai = true;
      thoiGianXacNhanTrangThai = millis();
    }
  ...
  if (doiXacNhanChuyenTrangThai && millis() - thoiGianXacNhanTrangThai > 5000) {
    doiXacNhanChuyenTrangThai = false;
  }
}